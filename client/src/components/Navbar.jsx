  ...
  return (
    <nav>
      <div className="w-full z-20 h-16 scroll px-4 lg:px-10 fixed top-0 flex flex-row justify-between items-center outline outline-1 outline-zinc-200 border-zinc-200 bg-white ">
        ...
        <div
          className="md:hidden  w-6 h-6 text-2xl cursor-pointer"
          onClick={() => {
            setOpen(!open);
          }}
        >
        ...
        <div
          className={` absolute w-full transition-all duration-300 top-16 left-0 ${
            open ? "h-[calc(100vh-64px)]" : "h-0"
          } md:h-16 overflow-y-scroll noscrollbar md:inset-0 md:w-auto md:relative`}
        >
          <div
            className={`md:p-0 pb-10 flex flex-col md:flex-row md:h-full min-h-full md:gap-7 font-nunito text-lg font-bold bg-white [&>*]:w-full [&>*]:md:w-auto [&>*]:grow [&>*]:flex [&>*]:items-center [&>*]:justify-center [&>*]:border-b [&>*]:md:border-b-0 [&>*]:py-4 [&>*:not(:last-child)]:md:py-0 text-center `}
          >
            <NavLink
              to={"/"}
              className={({ isActive }) =>
            ...
            {accessToken ? (
              ...
              >
                <img
                  src={
                    auth.userData?.img
                      ? serverUrl + "/" + auth.userData.img
                      : profilePlaceholder
                  }
                  alt=""
                  className="  inline rounded-full w-20 h-20 md:w-10 md:h-10 object-cover"
                />
                <p className="mt-1 text-center md:max-w-[130px] md:line-clamp-2">
                  {auth?.userData?.name}
                </p>
              </NavLink>
            ) : (
              <>
                <NavLink
                  to={"/login"}
                  className={({ isActive }) => {
                    return ` py-1 rounded-md font-light ${
                      isActive ? "text-primary" : "text-black"
                    }`;
                  }}
                >
