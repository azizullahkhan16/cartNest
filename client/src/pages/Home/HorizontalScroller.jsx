...
const HorizontalScroller = ({ category }) => {
  const { data, loading, msg } = useGetAxios(
    `/products?categories[]=${category}&limit=8`,
    axios,
    []
  );
  ...
  return (
    <>
      <div className=" mx-auto px-2 sm:px-4 py-10  max-w-[1520px] overflow-clip">
        <h2 className="text-xl font-semibold mb-2 font-nunito inline-block">
          {category}
        </h2>
        <Link
          to={`/products`}
          state={{ category }}
          className="px-4 py-2 mt-2 ml-8 border border-primary rounded-lg box-border inline-block hover:text-primary"
        >
          {"Show More >"}
        </Link>
