        ...
        data && (
          <>
            ...
            <div className="w-full px-2 py-4 border rounded-xl shadow-md ">
              ...
              {data.stock !== 0 ? (
                <p className="mt-2 inline-block w-fit px-2 rounded bg-green-500 text-white  text-lg">
                  in stock
                </p>
              ) : (
                <p className="mt-2 inline-block w-fit px-2 rounded bg-red-500 text-white  text-lg">
                  out of stock
                </p>
              )}
              <span className="ml-3">
                {data.stock >= 0 ? data.stock : "always available"}
              </span>{" "}
              <button
                className="text-xs border text-primary ml-8 border-primary rounded-full px-2 "
                onClick={() => {
                  setEditStock((p) => !p);
                }}
              >
                ...
                <div className="p-2 mt-2 border border-zinc-500 rounded-md w-full max-w-sm">
                  <div className="">
                    <input
                      ...
                      name="stock"
                      id="ADD"
                      checked={editStockData.mode === "ADD"}
                      onChange={() => {
                        setEditStockData((p) => {
                          return { ...p, mode: "ADD" };
                        });
                    ...
                    <input
                      type="text"
                      value={editStockData.value}
                      onChange={(e) => {
                        const value = e.target.value;
                        if (value.match("^\\d{0,3}$")) {
                          setEditStockData((p) => {
                            return { ...p, value: value ? parseInt(value) : 0 };
                          });
                        }
                      }}
                      disabled={editStockData.mode !== "ADD"}
                      className={`${
                        editStockData.mode !== "ADD" ? "hidden" : ""
                      } ml-2 border rounded-md outline-none border-primary w-10 px-1 `}
                    />
                  </div>
                  <div className="">
                    <input
                      ...
                      name="stock"
                      id="REMOVE"
                      checked={editStockData.mode === "REMOVE"}
                      onChange={() => {
                        setEditStockData((p) => {
                          return { ...p, mode: "REMOVE" };
                        });
                      }}
                      className="mr-2"
                    />
                    ...
                    <input
                      ...
                      className={`${
                        editStockData.mode !== "REMOVE" ? "hidden" : ""
                      } ml-2 border rounded-md outline-none border-primary w-10 px-1`}
                    />
                  </div>
                  <div className="">
                    <input
                      ...
                      name="stock"
                      id="SET"
                      checked={editStockData.mode === "SET"}
                      onChange={() => {
                        setEditStockData((p) => {
                          return { ...p, mode: "SET" };
                        });
                  ...
                  <div className="">
                    <input
                      ...
                      name="stock"
                      id="ALWAYS AVAILABLE"
                      checked={editStockData.mode === "ALWAYS AVAILABLE"}
                      onChange={() => {
                        setEditStockData((p) => {
                          return { ...p, mode: "ALWAYS AVAILABLE" };
                        });
                      }}
                      className="mr-2"
                    />
                  ...
                  <button
                    className="border  border-primary rounded-md px-2 w-24"
                    onClick={handleEditSubmit}
                  >
                    {" "}
                    {stockLoading ? (
                      <BsThreeDots className="mx-auto text-primary text-2xl" />
                    ) : (
                      "submit"
                    )}{" "}
                  </button>
