...
const Products = () => {
  ...
  const [filter, setFilter] = useState(() => {
    const obj = { All: true };
    const activeCategories = Categories.reduce((result, el) => {
      return { ...result, [el.name]: false };
    }, obj);
    if (state?.category) {
      ...
      <div className=" relative md:px-4  border-b ">
        <div className="w-full max-w-[1520px] mx-auto relative p-1 sm:p-4">
          ...
          <div className="relative inline-block md:absolute md:top-4 md:left-4 z-10">
            ...
            <div
              className="h-12 select-none w-fit px-3 flex justify-center items-center space-x-2 py-1 shadow-md rounded-full text-lg text-zinc-800 cursor-pointer"
              onClick={() => {
            ...
            >
              ...
              <div
                className={`rounded-full ${
                  filter.All ? "hidden" : ""
                } bg-primary text-base px-2 text-white`}
              >
                {Object.keys(filter).reduce((r, v) => {
                  return filter[v] ? r + 1 : r;
                }, 0)}
              </div>
              <BsChevronDown />
            </div>
