            ...
            {data.map((el, idx) => {
              return (
                <div className=" max-w-3xl border rounded-lg p-4 shadow-md">
                  ...
                  <p
                    className={`inline-block px-2 mt-2  rounded-md
                          ${
                            el.status === "Pending"
                              ? "bg-zinc-300"
                  ...
                  <div className="flex flex-col sm:flex-row gap-6">
                    <div className="border-t mt-2 grow min-w-[240px]">
                      {el.productsElements.map((el, idx) => {
                        return (
                          <div className="py-1 border-b" key={idx}>
                            <span>{idx + 1}</span>{" "}
                            <p className="ml-2 inline-block"> {el.name} </p>
                          </div>
                        );
                      })}
                    </div>
                    ...
                    <div className="flex sm:flex-col justify-end gap-3">
                      <Link
                        to={`/orders/${el._id}`}
                        className="w-20 px-4 box-content py-1 border font-medium hover:text-primary border-primary rounded text-center"
                      >
                        {" "}
                        Details
                      </Link>
