...
const Cart = () => {
  ...
  const { data, loading, error, setData } = useGetAxios(
    "/user/cart",
    privateAxios,
    []
  );
  ...
  useEffect(() => {
    ...
    if (data) {
      ...
      setAuth((p) => {
        return { ...p, userData: { ...p.userData, cart: data.cart } };
      });
    }
  }, [data, setAuth]);
    ...
    <div className="max-w-6xl relative grow p-1 sm:p-4 mx-auto flex flex-col lg:flex-row flex-wrap gap-4 ">
      <div className=" grow ">
        {error ? (
          ...
          <>
            <h1 className="text-2xl p-3 rounded-xl bg-zinc-100 ">
              {" "}
              your cart:
            </h1>
          ...
          <div className="flex  h-[calc(100vh-64px)] justify-center items-center ">
            <p className="text-xl font font-semibold text-red-500">
              {" "}
              Your cart is empty{" "}
            </p>
          </div>
      ...
      <div className=" max-w-sm min-w-fit w-full lg:w-auto  lg:grow relative mx-auto ">
        <div className=" sticky top-20 w-full  bg-zinc-800 rounded-lg py-6 px-3  text-white font-light flex flex-col space-y-5">
          ...
          <button
            className="w-full max-w-[240px] mx-auto py-1 rounded-md hover: bg-primary text-lg text-white font-normal font-nunito "
      ...
      {data && data.cart.length !== 0 && (
        <div
          className={`fixed py-10 px-4 top-0 left-0 overflow-y-auto h-screen w-full ${
            checkout ? "grid" : "hidden"
          }  grid-cols-1 backdrop-blur z-30`}
        >
          <div className="w-full m-auto p-3 rounded-xl relative max-w-xl bg-white border border-primary ">
            <button
              className="absolute -top-3 -right-3 rounded-full text-3xl text-primary bg-zinc-200"
              onClick={() => {
                setCheckout(false);
              }}
            >
            ...
            <ol className="list-decimal list-inside ">
              {data.products.map((el, idx) => {
                return (
                  <li
                    className="flex justify-between items-center px-3 even:bg-zinc-100 my-1"
                    key={idx}
                  >
                  ...
                  return (
                    <button
                      className={`border px-2 rounded-full ${
                        address === auth.userData.addresses[idx]
            ...
            <button
              className="py-1 px-3 my-4 mx-auto block text-primary border border-primary rounded-full text-lg font-semibold"
