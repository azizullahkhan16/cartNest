      ...
      {error ? (
        ...
        data && (
          <>
            ...
            <Link
              to={"/orders"}
              className="text-blue-400 pr-6  w-fit flex items-center gap-1 hover:text-blue-600"
            >
              {" "}
              <HiOutlineArrowSmLeft className="text-xl" /> <span>Orders</span>
            </Link>
            ...
            <div className="w-full px-4 py-1 border rounded-lg bg-white shadow-md mt-1">
              ...
              <div className="flex items-center justify-between w-full max-w-2xl flex-wrap">
                <p className="text-zinc-700">
                  <AiOutlineCalendar className="inline" />
              ...
              <div className="w-full flex items-center text-2xl text-zinc-400 gap-2 max-w-2xl">
                ...
                <div
                  className={`grow h-2  rounded-full ${
                    inStage("Processing", data.status)
                      ? "bg-primary"
                      : "bg-zinc-300"
                  }`}
                ></div>
                <FaGear
                  className={`${
                    inStage("Processing", data.status) ? "text-primary" : ""
                  } ${data.status === "Processing" ? "on" : ""}`}
                />
                <div
                  className={`grow h-2  rounded-full ${
                    inStage("Shipping", data.status)
                      ? "bg-primary"
                      : "bg-zinc-300"
                  }`}
                ></div>
                <FaTruck
                  className={`${
                    inStage("Shipping", data.status) ? "text-primary" : ""
                  } ${data.status === "Shipping" ? "ontruck" : ""}`}
                />
                <div
                  className={`grow h-2  rounded-full ${
                    inStage("Delivered", data.status)
                      ? "bg-primary"
                      : "bg-zinc-300"
                  }`}
                ></div>
                <FaCheckCircle
                  className={`${
                    data.status === "Delivered" ? "text-primary" : ""
                  }`}
                />
              </div>
            </div>
              ...
              <div className="mt-2 max-w-2xl [&>*:nth-child(even)]:bg-slate-100">
                ...
                <div className="flex justify-between items-center p-1 gap-3">
                  ...
                  <p className="whitespace-nowrap">
                    ${(data.shippingCost / 100).toFixed(2)}
                  </p>
                </div>
                <div className="flex justify-between p-1  ">
                  <p>Tax:</p>
